...
在 Kotlin 中对挂起友好且遵从结构化并发。没有积极的开拓者及其大量的工作，就不可能实现这一目标。您可以阅读 [Reactive Streams and Kotlin Flows](https://medium.com/@elizarov/reactive-streams-and-kotlin-flows-bfd12772cda4) 这篇文章来了解完成 Flow 的故事。

虽然有所不同，但从概念上讲，Flow *依然是*响应式流，并且它可以被转换为响应式（符合 TCK 规范）的发布者（Publisher），反之亦然。
...
集成模块包含 `Flow` 与 `Flow` 之间的转换，与 Reactor 的 `Context` 集成以及与一系列响应式实体配合使用的挂起友好的使用方式。
