...
-->在他们恰好被调用的任何线程中。我们在[线程与 Rx](#threads-with-rx) 这一小节中<!--
...
现在，这段代码中协程执行在了 Rx 的计算线程池斑并输出，只是<!--
...
-->线程切换操作符非常类似的。这在 Rx 中是正常默认的，因为操作符经常做一些非常小块的<!--
-->工作并且你必须做一些复杂处理来合并大量的运算符。然而，这对于协程来说是不常见的， 
